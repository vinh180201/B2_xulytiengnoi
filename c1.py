...
def audio_segment(signal_file_name, label_track_file):
    ...
    for label_track in label_track_list:
        try:
            ...
            signal = "data_output/" + label_track["label"] + "/" + file_number + '' + label_track["label"] + "" + str(i) + ".wav"
...
def get_MFCC(signal, sample_rate):
    ...
    delta_delta = librosa.feature.delta(mfccs, order=4, mode="nearest")
...
print('shape: ', dataset[2][0].shape)
print('label: ', dataset[2][1])

print("Example of MFFC result image")
mffc_example = dataset[2][0]
plt.xlabel("frames")
plt.ylabel("frequency")
plt.title(dataset[2][1])
